...
from unlearning_fl.utils import (
    ...
    get_cpu_memory,
    get_gpu_memory,
    save_results_as_pickle, compute_kl_div,
)
...
def main(cfg: DictConfig) -> None:  # pylint: disable=too-many-locals
    ...
    def get_evaluate_fn(
            ...
            starting_round: int, unlearned_model: bool = False,
    ) -> Callable[
        ...
        if unlearned_model or cfg.logging_train_accuracy:
            train_ds_for_test = load_client_datasets_from_files(
                dataset=dataset,
                sampled_client=int(0),
            ...
            global_summary_writer_on_train_ds = tf.summary.create_file_writer(
                os.path.join(save_path, "train0"))
        ...
        global_summary_writer = tf.summary.create_file_writer(
        ...
        def evaluate(
            ...
            if unlearned_model or cfg.logging_train_accuracy:
                loss, accuracy = model.evaluate(train_ds_for_test)

                preds = model.predict(train_ds_for_test)
                print("[Preds] ", preds[0])
                kl_div = compute_kl_div(preds.logits, num_classes)
                print("[kl_div] ", kl_div)
                with global_summary_writer_on_train_ds.as_default():
                    tf.summary.scalar("loss", loss, step=server_round)
                    tf.summary.scalar("accuracy", accuracy, step=server_round)
                    tf.summary.scalar("kl_div", kl_div, step=server_round)

            loss, accuracy = model.evaluate(
                test_ds,
            )
            if cfg.logging_test_accuracy:
    ...
    hp_unlearned_config = cfg.hp_unlearned_config
    beta_smoothing = cfg.beta_smoothing
    ...
    def client_fn(cid: str) -> TFClient:
        ...
        if algorithm in ["FedAvg"]:
            ...
            client_model = FedSmoothieModel(local_model)
            client_model.compile(
                optimizer=optimizer,
        ...
        client = TFClient(training_dataset, client_model, local_examples, algorithm, cid=int(cid), dataset=dataset, alpha_dir=alpha_dirichlet)
    ...
    if cfg.unlearned_model:
        sanitized_dataset_string = "_unlearned"

    eventual_cfg_algorithm = str(round(beta_smoothing, 2)) if algorithm in ["FedSmoothie"] else ""
    save_path_logging = os.path.join(
        "logging_results_unlearned",
        # "logging_hp_search",
        dataset + sanitized_dataset_string,
        model_name_string,
        algorithm + eventual_cfg_algorithm,
    ...
    if restart_from_checkpoint:
        ...
        if last_checkpoint:
            ...
            #     is_training=False,
            # )
            # server_model.evaluate(train_ds_for_test)
    ...
    if cfg.sanitized_dataset or cfg.unlearned_model:
        min_available_clients = (cfg.total_clients - 1)
    else:
        min_available_clients = cfg.total_clients
    ...
    strategy = instantiate(
        ...
        evaluate_fn=get_evaluate_fn(
            model_name, server_model, save_path_logging, dataset, starting_round, cfg.unlearned_model
        ),
        on_fit_config_fn=fit_config,
    )

    my_server = MyServer(strategy=strategy, starting_round=starting_round, unlearned_round=cfg.unlearned_round)
